"""K-fold CV for Gram-based FISTA Lasso (cv.py companion).

Per fold: precompute G_train, Xty_train on CPU (sparse-sparse mult on
the training rows), upload to GPU, run FISTA path, evaluate holdout MSE
via cheap dense Xβ on the holdout rows (CPU sparse @ dense β).
"""
...
from .fista_gram import compute_gram, fista_lasso_gram_path
...
def cv_fista_gram_sparse(
    ...
    """k-fold CV using Gram-based FISTA. X stays sparse on CPU; per-fold
    Gram is built on CPU and uploaded as dense to GPU.
    """
    if not HAS_JAX:
    ...
    # Build a default lambda grid using full-data Gram for λ_max
    if lambdas is None:
    ...
    for f, ho in enumerate(folds):
        ...
        # Lipschitz from G via power iteration (already in fista_lasso_gram_path)
        fits = fista_lasso_gram_path(
            G=G_j, Xty=Xty_j, lambdas=lambdas, n=len(y_tr),
            max_iter=max_iter, tol=tol, verbose=False,
        )
        # Evaluate holdout MSE using sparse SpMV + dense β (cheap)
