...
def health() -> dict:
    ...
    return {
        ...
        "methodology_version":    "v6 (regHAL-TMLE Delta-method, Li/Qiu/Wang/van der Laan 2025)",
        "hal_radii_complete":     hal_radii_complete,
        "xgb_radii_complete":     xgb_radii_complete,
        "xgb_radii_total":        20,
        "reghal_radii_complete":  len(list(REPO_ROOT.glob("reghal_shift_*km.csv"))),
        "combined_test_headline": {
            ...
            "near_field_km":      [1, 6],
            "near_field": {
                "note":   "regHAL-TMLE max_degree=2 produces a significant negative pooled estimate, but R=1 sensitivity (max_degree=1) yields null. Interpret near-field as inconclusive due to basis sensitivity.",
            },
            "description": "Inverse-variance-weighted combined test across 13 correlated radii. Targeted plug-in via the parametric efficient influence curve in HAL's implied finite-dimensional working model.",
            "xgb_gpu_comparator": {
                "psi":    6.08e-4,
                "ci_low": 2.97e-4,
                "ci_high": 9.19e-4,
