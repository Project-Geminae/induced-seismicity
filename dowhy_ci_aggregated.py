...
──────────────────────
Event-level (cluster-day) causal analysis WITH cluster-bootstrap CIs and
substantive refutation tests.

Same kernel as dowhy_ci.py but operates on the cluster-day aggregation
produced by causal_core.aggregate_panel_to_event_level. Cluster-bootstrap
resamples LOCATION-CLUSTERS (not wells), since location is the unit that
defines a row.

Output: dowhy_event_level_ci_<timestamp>.csv

Note: this script REPLACES the old dowhy_ci_aggregated.py which had the
same bugs as dowhy_ci.py (os.chdir hack, 3× CausalModel per bootstrap
iteration, brittle refutation thresholds) plus the median-pressure /
sum-of-fault-segments aggregation issues.
"""
...
OUTFILE         = Path(f"dowhy_event_level_ci_{TIMESTAMP}.csv")
...
def analyze_radius(R: int, window_days: int) -> dict | None:
    ...
    return {
        ...
        "n_clusters":        data[cc.COL_API].nunique(),
...
def main() -> None:
    log.info("Event-level causal analysis (window=%dd, n_boot=%d)", DEFAULT_WINDOW, N_BOOTSTRAP)
    rows = []
    t0 = time.time()
    for R in RADII:
    ...
    print("\n🎯  EVENT-LEVEL CAUSAL EFFECTS WITH CLUSTER-BOOTSTRAP 95% CIs")
    print("─" * 100)
